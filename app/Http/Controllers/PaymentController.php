...
class PaymentController extends Controller
{
    ...
    {
        $paypal_data = Settings::all()->where('type', 'PayPal')->pluck('value', 'name');
    ...
    public function index(Request $request)
    {
        ...
        $data['paypal_status'] = Settings::all()->where('name', 'paypal_status')
                                ->where('type', 'PayPal')->first();

        $data['stripe_status'] = Settings::all()->where('name', 'stripe_status')
                                ->where('type', 'Stripe')->first();
        $data['crypto_status'] = Settings::all()->where('name', 'crypto_status')
                                ->where('type', 'WalletConnect')->first();
        $data['euro_wallet']   = Wallet::where('user_id', Auth::user()->id)->where('wallet_type', 'currency')->first();
        $data['boxa_wallet']   = Wallet::where('user_id', Auth::user()->id)->where('wallet_type', 'crypto')->first();

        $data['banks'] = Bank::getAll()->where('status', 'Active')->count();
        if ($request->isMethod('post')) {
            $number_of_guests = $request->number_of_guests + $request->number_of_children + $request->number_of_infants;
            ...
            Session::put('payment_number_of_pets', $request->number_of_pets);
            ...
            $number_of_pets   = Session::get('payment_number_of_pets');
        ...
        $data['number_of_pets']   = $number_of_pets;
        ...
        $data['price_list']    = json_decode($this->helper->getPrice(
            $data['property_id'], 
            $data['checkin'], 
            $data['checkout'], 
        ...
        $data['currencyDefault']  = $currencyDefault = Currency::all()->firstWhere('default',1);
    ...
    {
        $paypal_credentials = Settings::all()->where('type', 'PayPal')->pluck('value', 'name');
        $currencyDefault    = Currency::all()->where('default', 1)->first();
        $price_list         = json_decode($this->helper->getPrice(
            $request->property_id, 
            $request->checkin, 
            $request->checkout, 
        ...
        if ($request->payment_method == 'Boxa_Wallet') {
            return redirect('payments/boxa-wallet-payment');
        } elseif ($request->payment_method == 'Currency_Wallet') {
            return redirect('payments/currency-wallet-payment');
        } elseif ($request->payment_method == 'WalletConnect') {
            return redirect('payments/wallet-connect');
    ...
    public function getDataForBooking() {
        ...
        $data['currencyDefault']  = $currencyDefault = Currency::all()->where('default', 1)->first();
    ...
    public function boxaWalletPayment(Request $request)
    {
        $data = $this->getDataForBooking();
        $stripe                   = Settings::where('type', 'Stripe')->pluck('value', 'name');
        $data['crypto_wallet'] = wallet::where('user_id', Auth::user()->id)->where('wallet_type', 'crypto')->first();
        $data['publishable']      = $stripe['publishable'];
        return view('payment.boxa_wallet', $data);
    }

    public function boxaWalletRequest(Request $request)
    {
        $currencyDefault = Currency::all()->where('default', 1)->first();
        $crypto_wallet = wallet::where('user_id', Auth::user()->id)->where('wallet_type', 'crypto')->first();
        if ($request->isMethod('post')) {
            
            if (isset($crypto_wallet)) {
                $id            = Session::get('payment_property_id');
                $booking_id    = Session::get('payment_booking_id');
                $booking_type  = Session::get('payment_booking_type');
                ...
                if ($crypto_wallet->balance >= $price_list->total_boxa_without_format) {
                    $pm    = PaymentMethods::where('name', 'BoxaWallet')->first();
                    $data  = [
                    ...
                    $crypto_wallet->balance = $crypto_wallet->balance - $price_list->total_boxa_without_format;
                    $crypto_wallet->save();
    ...
    public function currencyWalletPayment(Request $request)
    {
        $data = $this->getDataForBooking();
        $stripe                   = Settings::where('type', 'Stripe')->pluck('value', 'name');
        $data['currency_wallet'] = wallet::where('user_id', Auth::user()->id)->where('wallet_type', 'currency')->first();
        $data['currentCurrency'] = $this->helper->getCurrentCurrency();
        $data['publishable']      = $stripe['publishable'];
        return view('payment.currency_wallet', $data);
    }

    public function currencyWalletRequest(Request $request)
    {
        $currencyDefault = Currency::all()->where('default', 1)->first();
        $currency_wallet = wallet::where('user_id', Auth::user()->id)->where('wallet_type', 'currency')->first();
        if ($request->isMethod('post')) {
            
            if (isset($currency_wallet)) {
                $id            = Session::get('payment_property_id');
                $booking_id    = Session::get('payment_booking_id');
                $booking_type  = Session::get('payment_booking_type');
                $price_list    = Session::get('payment_price_list');
                $price_eur     = $this->helper->convert_currency($this->helper->getCurrentCurrencyCode(), $currencyDefault->code, $price_list->total);
                // dd($price_list);
                if ($currency_wallet->balance >= $price_list->total) {
                    $pm    = PaymentMethods::where('name', 'Wallet')->first();
                    $data  = [
                        'property_id'      => Session::get('payment_property_id'),
                        'checkin'          => Session::get('payment_checkin'),
                        'checkout'         => Session::get('payment_checkout'),
                    ...
                    $currency_wallet->balance = $currency_wallet->balance - $price_list->total;
    ...
    {
        if (!Session::get('payment_property_id')) {
            return redirect('/');
        }
        $data = $this->getDataForBooking();        
        return view('payment.walletConnect', $data);
    }
    ...
    public function walletRequest(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            if (isset($request->transactionHash)) {
                ...
                // if ($response->isSuccessful()) {
                if (true) {
                    $pm    = PaymentMethods::where('name', 'Crypto')->first();
                    $data  = [
                        'property_id'      => Session::get('payment_property_id'),
                        'checkin'          => Session::get('payment_checkin'),
                        'checkout'         => Session::get('payment_checkout'),
    ...
    public function bankPayment(Request $request) {
        ...
        $attachment = $this->helper->uploadSingleFile($_FILES["attachment"], 'uploads/booking/');
        if ($attachment) {
    ...
    public function store($data)
    {
        ...
        $booking->pets              = $data['number_of_pets'];
        ...
        $booking->pets_charge       = $this->helper->convert_currency('', $currencyDefault->code, $data['price_list']->pets_price);
        ...
        if ($data['payment_method_id'] == 5) {
            $booking->service_charge    = $service_fee  = 0;
        } else {
        ...
        $booking->boxa_conversion   = Session::get('BOXA_PRICE');
       ...
       $booking->date_with_price   = json_encode($allData);
        ...
        if ($booking->status =='Accepted') {
            $companyName = Settings::all()->where('type', 'general')->where('name', 'name')->first()->value;
        ...
        Session::forget('payment_number_of_pets');
    ...
    public function update($data)   {
        ...
        $booking->save();
        Session::put('user_boxa_account', null);
        Session::put('user_boxa_balance', null);
    ...
    {
        $walletBalance  = Wallet::where('user_id',$booking->host_id)->where('wallet_type', 'currency')->first();
        $default_code   = Currency::all()->firstWhere('default',1)->code;
        $wallet_code    = Currency::all()->firstWhere('id', $walletBalance->currency_id)->code;
        $balance        = ( 
            $walletBalance->balance + 
            $this->helper->convert_currency($default_code, $wallet_code, $booking->total)  - 
            $this->helper->convert_currency($default_code, $wallet_code, $booking->service_charge) - 
            $this->helper->convert_currency($default_code, $wallet_code, $booking->accomodation_tax) - 
            $this->helper->convert_currency($default_code, $wallet_code, $booking->iva_tax) 
        );
        Wallet::where(['user_id' => $booking->host_id])->where('wallet_type', 'currency')->update(['balance' => $balance]);
    }
