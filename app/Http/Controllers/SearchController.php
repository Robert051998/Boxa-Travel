...
class SearchController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $data['infants']    = $request->input('infants');
        ...
        $minPrice = Settings::all()->where('name', 'min_search_price')->first()->value;
        $maxPrice = Settings::all()->where('name', 'max_search_price')->first()->value;
        $data['default_min_price'] = $this->helper->convert_currency(Currency::all()->firstWhere('default')->code, '', $minPrice);
        $data['default_max_price'] = $this->helper->convert_currency(Currency::all()->firstWhere('default')->code, '', $maxPrice);
        if (!$data['min_price']) {
    ...
    function searchResult(Request $request)
    {
        ...
        if(!empty($checkin)){
            $checkin = date_create_from_format('m-d-Y',$checkin)->format('Y-m-d');
        }
        if(!empty($checkout)){
            $checkout = date_create_from_format('m-d-Y',$checkout)->format('Y-m-d');
        }
        ...
        if ($pets > 0) {
            $properties_where['properties.pets_allowed'] = 1;
        } 
        // else {
        //     $properties_where['properties.pets_allowed'] = 0;
        // }
        ...
        if (isset($full_address)) {
            ...
            $properties = $properties->whereHas('property_address', function ($query) use ($minLat, $maxLat, $minLong, $maxLong) {
