...
class EmailController extends Controller
{
    ...
    public function welcome_email($user)
    {
        ...
        if($emailConfig['driver']=='smtp' && $emailConfig['email_status']==1){
            ...
            Mail::send('emails.email_confirm_template', $data, function($message) use($data,$subject,$content) {
                $message->from('no-reply@boxatravel.com','BoxaTravel');
                //$message->bcc(['service@boxatravel.com']);
    ...
    public function forgot_password($user)
    {
        ...
        $data['subject']    = "Boxa Travel Password Reset Request";
    ...
    public function change_email_confirmation($user)
    {
        ...
        $data['subject']     = "Confirm Your Email Address with Boxa Travel";
        $data['view']        = resource_path('views/sendmail/email_confirm.blade.php');
        $data['link']        = $data['url'].'users/confirm_email?code='.$data['token'];
        $data['link_text']   = trans('messages.email_template.confirm_email');
        $englishTemplate = EmailTemplate::where(['temp_id' => 5, 'lang_id' => '1', 'type' => 'email'])->select('subject', 'body')->first();
        ...
        if(env('APP_MODE', '') != 'test'){
             if($emailConfig['driver']=='smtp' && $emailConfig['email_status']==1){
                Mail::send('emails.email_confirm', $data, function($message) use($user) {
                    $message->from('no-reply@boxatravel.com','BoxaTravel');
                    //$message->bcc(['service@boxatravel.com']);
                    $message->to($user->email, $user->first_name)->subject('Confirm Your Email Address with Boxa Travel');
                 });
            }else if($emailConfig['driver']=='sendmail'){
              $this->sendPhpEmail($data,$emailConfig);
            }
    ...
    public function account_activated($user)
    {
        $emailSettings   = Settings::all()->where('type', 'email')->toArray();
        $emailConfig     = $this->helper->key_value('name', 'value', $emailSettings);
        $adminDetails    = Admin::where('status', 'active')->first();
        ...
        $data['url']       = url('/').'/';
         ...
         $data['user_name']    = '';
         $subject='Boxa Travel Account Activation';
      
        if (env('APP_MODE', '') != 'test') {
            if ($emailConfig['driver']=='smtp' && $emailConfig['email_status']==1) {
                Mail::send('emails.account_activated', $data, function ($message) use ($user, $data, $subject) {
                    $message->from('no-reply@boxatravel.com','BoxaTravel');
                    $message->bcc(['service@boxatravel.com']);
                    $message->to($user->email, $user->first_name)->subject($subject);
                });
    ...
    public function account_preferences($user_id, $type = 'update', $updateTime )
    {
        $emailSettings   = Settings::all()->where('type', 'email')->toArray();
        $emailConfig     = $this->helper->key_value('name', 'value', $emailSettings);
        $adminDetails    = Admin::where('status', 'active')->first();
        ...
        $details = UserDetails::where('user_id', $user->id)->pluck('value', 'field')->toArray();
        ...
        if ($type == 'update') {
            ...
            $subjectFromDB  = str_replace('{site_name}', SITE_NAME, $subjectFromDB);
            $bodyFromDB     = str_replace('{site_name}', SITE_NAME, $bodyFromDB);
            $bodyFromDB     = str_replace('{first_name}', $user->first_name.' '.$user->last_name, $bodyFromDB);
            $bodyFromDB     = str_replace('{email}', $user->email, $bodyFromDB);
            $bodyFromDB     = str_replace('{phone}', $user->formatted_phone, $bodyFromDB);
            $bodyFromDB     = str_replace('{date_time}', $updateTime, $bodyFromDB);
           // $details = UserDetails::where('user_id', Auth::user()->id)->pluck('value', 'field')->toArray();
            $bodyFromDB     = str_replace('{address}', $details['live'], $bodyFromDB);
            // dd($details);
    ...
    {        
        ...
        $data['url']        = url('/').'/';
        $data['logo']       = url('images/boxa-travel-logo.svg');
        ...
        $bodyFromDB     = str_replace('{booking_code}', $data['result']['code'], $bodyFromDB);
        ...
        $bodyFromDB     = str_replace('{end_date}', onlyFormat($data['result']['end_date']), $bodyFromDB);
    ...
    public function getPropertyAddress($property )
    {        
        
        $property_objected = Properties::where('id', $property['id'])->first();
        $addres = $property_objected->property_address->address_line_1 . ', ' . 
            $property_objected->property_address->city.', '.$property_objected->property_address->country;
        return $addres;
    }
    ...
    {        
        ...
        $subjectFromDB  = str_replace('{booking_code}', $data['result']['code'], $subjectFromDB);
        $subjectFromDB  = str_replace('{property_name}', $data['result']['properties']['name'], $subjectFromDB);
        $bodyFromDB     = str_replace('{owner_first_name}', $booking->host->first_name, $bodyFromDB);
        $bodyFromDB     = str_replace('{user_first_name}', $data['result']['users']['first_name'], $bodyFromDB);
        $bodyFromDB     = str_replace('{total_night}', $data['result']['total_night'], $bodyFromDB);
        $bodyFromDB     = str_replace('{total_amount}', $data['result']['currency_code'].' '.$data['result']['total'], $bodyFromDB);
        if ($data['result']['total_night']>1) {
            $myStr = 'nights';
        }
        if ($data['result']['total_night']=1) {
        ...
        $bodyFromDB     = str_replace('{start_date}', $checkinDate, $bodyFromDB);
        $bodyFromDB     = str_replace('{booking_code}', $data['result']['code'], $bodyFromDB);
        $bodyFromDB     = str_replace('{property_type}', $data['result']['properties']['property_type_name'], $bodyFromDB);
        $bodyFromDB     = str_replace('{number_of_rooms}', $data['result']['properties']['bedrooms'], $bodyFromDB);
        $bodyFromDB     = str_replace('{property_address}', $this->getPropertyAddress($data['result']['properties']), $bodyFromDB);
        $bodyFromDB     = str_replace('{payment_method}', PaymentMethods::find($data['result']['payment_method_id'])->name ?? 'Not selected yet.', $bodyFromDB);
    ...
    public function bookingCancellation($booking_id)
    {
        ...
        $bodyFromDB     = str_replace('{property_type}', $data['result']['properties']['property_type_name'], $bodyFromDB);
        $bodyFromDB     = str_replace('{number_of_rooms}', $data['result']['properties']['bedrooms'], $bodyFromDB);
        $bodyFromDB     = str_replace('{start_date}', $data['result']['start_date'], $bodyFromDB);
        $bodyFromDB     = str_replace('{property_address}', $this->getPropertyAddress($data['result']['properties']), $bodyFromDB);
        $bodyFromDB     = str_replace('{total_night}', $data['result']['total_night'], $bodyFromDB);
        $bodyFromDB     = str_replace('{total_amount}', $data['result']['currency_code'].' '.$data['result']['total'], $bodyFromDB);
        $data['subject'] = $subject = $subjectFromDB;
        ...
        if(env('APP_MODE', '') != 'test'){
            if($emailConfig['driver']=='smtp' && $emailConfig['email_status']==1){
                Mail::send('emails.booking_cancel_template', $data, function($message) use($user, $data,$subject,$content) {
                    ...
                    // host user == logged in user
                    if($logged_user->id==$user->id){
                        $message->to( $data['result']['users']['email'],  $data['result']['users']['first_name'])->subject($subject);
                    }else{
                        $message->to($user->email, $user->first_name)->subject($subject);
                    }
    ...
    {
        return Settings::all()->where('type', 'general')->where('name', 'default_language')
                                ->first()
                                ->value;
    ...
    public function bankAdminNotify($bookingId, $checkInDate) {
        $emailSettings   = Settings::all()->where('type', 'email')->toArray();
        $receiver = Settings::all()->where('type','general')->whereIn('name',['email','name'])->pluck('value','name');
        $emailConfig     = $this->helper->key_value('name', 'value', $emailSettings);
        $adminDetails    = Admin::where('status', 'active')->first();
        $emailConfig['email_address']= $adminDetails->email;
        $emailConfig['username']     = $adminDetails->username;
