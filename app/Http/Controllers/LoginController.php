...
class LoginController extends Controller
{
    ...
    {
        $data['social'] = Settings::all()->where('type','social')->pluck('value','name');
        ...
        if ($validator->fails()) {
            ...
            if (!empty($users)) {
                if ($users->status != 'Inactive') {
                    ...
                    $this->helper->one_time_message('error', "messages.error.user_inactive");
                    return redirect('login');
                } else {
                    $this->helper->one_time_message('error', trans('messages.error.login_info_error'));
                    return redirect('login');
                ...
                $this->helper->one_time_message('error', trans('messages.error.account_associated'));
                    return redirect('login');
            }
