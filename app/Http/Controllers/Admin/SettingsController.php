...
class SettingsController extends Controller
{
    ...
    public function general(Request $request)
    {
        if (! $request->isMethod('post')) {
            $general          = Settings::all()->whereIn('type', ['general','googleMap'])->toArray();
    ...
    {
        if (!$request->isMethod('post')) {
            $preferences          = Settings::all()->where('type','preferences')->toArray();
    ...
    {
        if (! $request->isMethod('post')) {
             $photos = Settings::all()->where('type', 'photos')->toArray();
    ...
    {
        if (! $request->isMethod('post')) {
             $general         = Settings::all()->where('type', 'email')->toArray();             
             $data['result']  = $this->helper->key_value('name', 'value', $general);             
    ...
    {
        if (! $request->isMethod('post')) {
              $general = Settings::all()->where('type', 'join_us')->toArray();
              $data['result'] = $this->helper->key_value('name', 'value', $general);
    ...
    public function socialLogin(Request $request) {
        if (! $request->isMethod('post')) {
            $general = Settings::all()->where('type', 'social')->toArray();
