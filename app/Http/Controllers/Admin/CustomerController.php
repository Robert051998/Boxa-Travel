...
class CustomerController extends Controller
{
...

    {
        ...
        $pref = Settings::all()->where('type', 'preferences');
        if (! empty($pref)) {
    ...
    public function sendBoxaToWallet(Request $request, $id)
    {
        if (! $request->isMethod('post')) {
            $data['wallet']      = 'active';
            $data['user']         = DB::table('users')->where('id',$id)->first();
            ...
            $data['total']         = DB::table('boxa_logs')->where('user_id',$id)->get()->sum('amount');

            return view('admin.customerdetails.send_boxa_to_wallet', $data);            
        } else {
            $wallet        = Wallet::where('user_id',$id)->where('wallet_type', 'crypto')->first();
            $wallet->balance = $wallet->balance + $request->amount;
            $wallet->save();
            $boxalog = new Boxalog;
            $boxalog->amount = $request->amount;
            $boxalog->user_id = $id;
            $boxalog->save();


            $this->helper->one_time_message('success', 'Amount added to wallet');

                return redirect('admin/customer/wallet/'.$id);
        }
    }
}
