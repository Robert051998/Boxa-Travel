...
class UserController extends Controller
{
    ...
    public function create(Request $request, EmailController $email_controller)
    {
        ...
        if ($validator->fails()) {
           ...
           // $email_controller->signup_confirmation($user);
    ...
    public function dashboard()
    {
        ...
        $data['currency_wallet'] = wallet::where('user_id', $user_id)->where('wallet_type', 'currency')->first();
        $data['crypto_wallet']  = wallet::where('user_id', $user_id)->where('wallet_type', 'crypto')->first();
        ...
        $bookings = Bookings::select('payment_method_id', 'currency_code',
            DB::raw('(total - service_charge - iva_tax - accomodation_tax) as total'), 'created_at', 'transaction_id', DB::raw('1 as type'))
            ->where(['host_id' => $user_id, 'status' => 'Accepted']);

        $trips = Bookings::select('payment_method_id', 'currency_code', 'total', 'created_at', 'transaction_id', DB::raw('-1 as type'))
            ->where(['user_id' => $user_id, 'status' => 'Accepted']);

        $data['transactions'] = Withdrawal::with('payment_methods','currency')
            ->select('payment_method_id', 'currency_id', 'amount', 'created_at', 'payout_id as transaction_id', DB::raw('0 as type'))
    ...
    public function profile(Request $request, EmailController $email_controller)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            if ($validator->fails()) {
                ...
                $updateTime = dateFormat($user->updated_at);
                $email_controller->account_preferences($user->id,$type = "default_update", $updateTime);
                if ($new_email == 'yes') {
    ...
    public function media(Request $request)
    {
        ...
        if (isset($_FILES["photos"]["name"]) && $request->file('photos')) {           
            $aws_enabled = env('AWS_ACCESS_KEY_ID');
            $s3filePath = 'images/profile/'.Auth::user()->id.'/';
            $image =  $request->file('photos')[0];                        
    ...
    public function show(Request $request)
    {
        ...
        $data['users_properties'] = Properties::where('host_id', $request->id)->where('status','Listed')->get();
    ...
    public function confirmEmail(Request $request, EmailController $email_controller)
    {
        ...
        if (Auth::check()) {
            if ($password_resets->count() && Auth::user()->email == $password_resets->first()->email) {
                ...
                if(!$user_verification){
                    $user_verification = new UsersVerification;
                    $user_verification->user_id = $data['result']->id;
                }
            ...
            if ($hours >= 1) {
                $password_resets->delete();
                $this->helper->one_time_message('error', trans('messages.login.token_expired'));
                return redirect('login');
            }
            $data['result'] = User::whereEmail($password_result->email)->first();
            $data['token']  = $request->code;
            ...
            $password_resets->delete();
            $email_controller->account_activated($user);
            $this->helper->one_time_message('success', trans('messages.profile.email_confirmed'));
            return redirect('login');
        }else{
            $this->helper->one_time_message('error', trans('messages.login.token_expired'));
    ...
    public function facebookConnect(Request $request)
    {
        ...
        $verification = UsersVerification::where('user_id',Auth::user()->id)->get()->first();        
    ...
    public function reviewsByYou(Request $request)
    {
        ...
        $data['reviewsToWrite'] = Bookings::with('properties','host','users')->whereRaw('DATEDIFF(now(),end_date) <= 14')
            ->whereRaw('DATEDIFF(now(),end_date)>=0')
