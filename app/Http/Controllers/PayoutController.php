...
class PayoutController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['currentCurrency'] = Currency::all()->firstWhere('code',
            Session::get('currency') ?? $this->helper->getCurrentCurrencyCode());
        $data['walletBalance'] = Wallet::where(['user_id' => Auth::user()->id])->where('wallet_type', 'currency')->first();
    ...
    public function payoutList(PayoutListDataTable $dataTable)
    {
        ...
        {
            $userId            = Auth::user()->id;
            $walletMoney       = Wallet::where('user_id', $userId)->where('wallet_type', 'currency')->first();
