...
class Common {
    ...
    {
        $from_rate = Currency::all()->firstWhere('code',$from)->rate;
        $to_rate   = Currency::all()->firstWhere('code',$to)->rate;
    ...
    public function getCurrentCurrencySymbol() {
        return Session::get('currency') ? Currency::code_to_symbol(Session::get('currency'))
            : Currency::all()->firstWhere('default', 1)->symbol;
    }
    public function getCurrentCurrencyCode() {
        return Session::get('currency') ?? Currency::all()->firstWhere('default', 1)->code;
    }

    public function getCurrentCurrency() {
        return Session::get('currency') ? Currency::firstWhere('code', Session::get('currency'))
            : Currency::all()->firstWhere('default', 1);
    }

    public function getPrice($propertyId, $checkIn, $checkOut, $guestCount, $petsCount = 0, $force = false)
    {
        ...
        $result['property_default'] = [
            ...
            'symbol' => Currency::all()->firstWhere('code', $property->property_price->currency_code)->symbol,
            'rate' => Currency::all()->firstWhere('code', $property->property_price->currency_code)->rate,
        ...
        foreach($allDate as $key => $value) {
            $allDate[$key]['boxa_price'] = getBoxaPrice($value['original_price']);
        } 
        ...
        $result['total_nights_text'] = $countDays>1?'s':'';
        ...
        $result['host_fee_boxa'] = getBoxaPrice($result['host_fee']);
        ...
        if ($petsCount > 0) {
            $result['pets_price'] = $property->property_price->pet_price * $petsCount;
        }
        ...
        $result['total_without_discount'] = $result['service_fee'] + $result['total_night_price'] + $result['additional_guest'] + $result['pets_price']
            + $result['security_fee'] + $result['cleaning_fee'] + $result['iva_tax'] + $result['accomodation_tax'];

        $discount_value = 0;
        $user_boxa_balance = Session::get('user_boxa_balance');
        if (Session::get('user_boxa_account')) {        
            $discount_rows = Discounts::getAll();
            foreach($discount_rows as $discount_row) {
                if ($discount_row->amount <= $user_boxa_balance) {
        ...
        // dd();
        if ($discount_value > 0 ) {
            $result['discount'] = numberFormat(($result['total_without_discount'] *($discount_value / 100)), $decimals);;
        }

        $result['total'] = $result['service_fee'] + $result['total_night_price'] + $result['additional_guest'] + $result['pets_price']
            + $result['security_fee'] + $result['cleaning_fee'] + $result['iva_tax'] + $result['accomodation_tax'] - $result['discount'];
        $result['total_for_boxa'] = $result['total_night_price'] + $result['additional_guest'] + $result['pets_price']
            + $result['security_fee'] + $result['cleaning_fee'] + $result['iva_tax'] + $result['accomodation_tax'] - $result['discount'];
        $result['subtotal'] = $result['total'];
        $result['subtotal_boxa'] = getBoxaPrice($result['subtotal']);
        
        $result['total_night_price_with_symbol'] = moneyFormat($symbol, numberFormat($result['total_night_price'], $decimals));
        $result['total_night_price_boxa'] = getBoxaPrice($result['total_night_price']);
        $result['service_fee_with_symbol'] = moneyFormat($symbol, numberFormat($result['service_fee'], $decimals));
        $result['service_fee_boxa'] = getBoxaPrice(0);
        $result['total_with_symbol'] = moneyFormat($symbol, numberFormat($result['total'], $decimals));
        $result['total_boxa'] = getBoxaPrice($result['total_for_boxa']);
        $result['total_boxa_without_format'] = getBoxaPriceWihtoutFormat($result['total_for_boxa']);
        $result['iva_tax_with_symbol'] = moneyFormat($symbol, numberFormat($result['iva_tax'], $decimals));
        $result['iva_tax_boxa'] = getBoxaPrice($result['iva_tax']);
        $result['accomodation_tax_with_symbol'] = moneyFormat($symbol, numberFormat($result['accomodation_tax'], $decimals));
        $result['accomodation_tax_boxa'] = getBoxaPrice($result['accomodation_tax']);;
        $result['additional_guest_fee_with_symbol'] = moneyFormat($symbol, numberFormat($result['additional_guest'], $decimals));
        $result['additional_guest_boxa'] = getBoxaPrice($result['additional_guest']);;
        $result['pets_price_with_symbol']   = moneyFormat($symbol, numberFormat($result['pets_price'], $decimals));
        $result['pets_price_boxa']              = getBoxaPrice($result['pets_price']);;
        $result['security_fee_with_symbol']     = moneyFormat($symbol, numberFormat($result['security_fee'], $decimals));
        $result['security_fee_boxa']            = getBoxaPrice($result['security_fee']);;
        $result['cleaning_fee_with_symbol']     = moneyFormat($symbol, numberFormat($result['cleaning_fee'], $decimals));
        $result['cleaning_fee_boxa']            = getBoxaPrice($result['cleaning_fee']);;
        $result['per_night_price_with_symbol']  = moneyFormat($symbol, numberFormat($result['total_night_price'] / $countDays, $decimals));
        $result['per_night_price_boxa']         = getBoxaPrice($result['total_night_price'] / $countDays);
        $result['discount_with_symbol']         = moneyFormat($symbol, numberFormat($result['discount'], 2));
        $result['discount_boxa'] = getBoxaPrice($result['discount']);
    ...
    public function uploadSingleFile($file, $path = 'uploads/') {
