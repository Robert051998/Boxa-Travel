...
class Payouts extends Model
{
    ...
    public function getTotalPenaltyAmountAttribute()
    {
        ...
        if ($penalty != 0) {
            $rate = Currency::all()->firstWhere('code', $this->attributes['currency_code'])->rate;

            $base_amount = $penalty / $rate;

            $default_currency = Currency::all()->firstWhere('default_currency', 1)->code;

            $now_rate = Currency::all()->where('code',(Session::get('currency')) ? Session::get('currency') : $default_currency)->first()->rate;
    ...
    public function getCurrencySymbolAttribute()
    {
        ...
        return Currency::all()->firstWhere('code', $default_code)->symbol;
    }
