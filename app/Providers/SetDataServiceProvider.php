...
class SetDataServiceProvider extends ServiceProvider
{
    ...
    public function currency()
    {
        ...
        $currencies = Currency::all()->where('status', '=', 'Active');
        ...
        if(!\Session::get('currency')) {
            try {
                if($_SERVER["REMOTE_ADDR"]){
                    ...
                    $default_currency = Currency::all()->where('status', '=', 'Active')->where('code', '=', $remoteData['geoplugin_currencyCode'])->first();
                    $default_country = $remoteData['geoplugin_countryCode'];
                }
            } catch(\Exception $e) {
                $default_currency = Currency::all()->firstWhere('default', '=', '1');
            }
            //$default_currency= json_decode(json_encode(['code'=>'boxa','symbol'=>'b']));
        } else {
            $default_currency = Currency::all()->firstWhere('code', \Session::get('currency'));
        }
        ...
        if(!isset($default_country)) {
            $first_country = Country::all()->first();
            if ($first_country) {
                $default_country = $first_country->short_name;
            }
        }
    ...
    public function settings()
    {
        ...
        if (!empty($settings)) {
            ...
            if (!empty($general['logo']) && file_exists(public_path('front/images/logos/'. $general['logo']))) {
                $logo = url('front/images/logos/'. $general['logo']);
            } else {
                $logo = env('APP_LOGO_URL') != '' ? env('APP_LOGO_URL') : url('front/images/logos/logo.png');
            }
            if (!defined('LOGO_URL')) {
            ...
            // dd($crypto_boxa_price);
            if ($crypto_boxa_price) {
        ...
        if ( isset($general['logo']) && !empty($banner) && file_exists(public_path('front/images/logos/'. $general['logo'])) )
        {
