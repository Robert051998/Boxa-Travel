...
class TransactionDataTable extends DataTable
{
    public function ajax()
    {
        ...
        ->addColumn('method', function ($transaction) {            
            return ($transaction->method == 'Crypto' ? 'Boxa' : $transaction->method);
        })
        ...
        ->addColumn('amount', function ($transaction) {
            if ($transaction->method == 'Crypto') {
                return '<span class="'. (($transaction->type < 1) ? 'text-danger' : 'text-success') . '"><a target="_blank" href="' . ($transaction->transaction_id) . '">' . '...'.substr($transaction->transaction_id, -6) . '</a></span>';
            } else {
                return '<span class="'. (($transaction->type < 1) ? 'text-danger' : 'text-success') . '"> '. (($transaction->type < 1) ? '-' : '+') . ' ' . Session::get('symbol') . currency_fix($transaction->amount, $transaction->code) . '</span>';
            }             
        })
    ...
    public function query()
    {
        ...
        })->select(['withdrawals.id as id',  'withdrawals.amount AS amount', 'withdrawals.currency_id AS c', 'currency.symbol AS symbol', 'currency.code AS code', 'payment_methods.name AS method', 'withdrawals.created_at AS date', 'withdrawals.payout_id AS transaction_id', DB::raw('0 as type')])->where('withdrawals.user_id', '=', $user_id)->where('withdrawals.status', '=', 'Success');
        ...
        })->select(['bookings.id as id', DB::raw('(bookings.total - bookings.service_charge -bookings.accomodation_tax - bookings.iva_tax) as amount'), 'bookings.currency_code AS c', 'currency.symbol AS symbol', 'currency.code AS code','payment_methods.name AS method', 'bookings.created_at AS date', 'bookings.transaction_id AS transaction_id', DB::raw('1 as type')])->where('bookings.host_id', '=', $user_id)->where('bookings.status', '=', 'Accepted');
        ...
        $trips = Bookings::join('currency', function ($join) {
            ...
            $join->on('bookings.payment_method_id', '=', 'payment_methods.id');
        })->select(['bookings.id as id', 'bookings.total as amount', 'bookings.currency_code AS c', 'currency.symbol AS symbol', 'currency.code AS code','payment_methods.name AS method', 'bookings.created_at AS date', 'bookings.transaction_id AS transaction_id', DB::raw('-1 as type')])->where('bookings.user_id', '=', $user_id)->where('bookings.status', '=', 'Accepted');
    ...
    public function html()
    {
        ...
        ->parameters(dataTableOptions(['searching' => false, 'ordering' => false, 'bPaginate' => false]));
    }
