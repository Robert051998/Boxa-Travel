...
if(!function_exists('setDateForDb')) {
    function setDateForDb($value = null)
    {
        ...
        $separator   = Settings::all()->firstWhere('name', 'date_separator')->value;
        $date_format = Settings::all()->firstWhere('name', 'date_format_type')->value;;
...
{
    $timezone       = '';
    $timezone       = Settings::all()->where('type', 'preferences')->where('name', 'dflt_timezone')->first()->value;
    ...
    $preferenceData = Settings::all()->where('type','preferences')->whereIn('name', ['date_format_type', 'date_separator'])->toArray();
...
function twilioSendSms($toNumber,$messages)
{
    ...
    try {
        ...
        $phoneSms        = Settings::all()->where('type','twilio')->whereIn('name', ['twilio_sid', 'twilio_token','formatted_phone'])->pluck('value', 'name')->toArray();
...
{
    $preferenceData = Settings::all()->whereIn('name', ['date_format_type', 'date_separator'])->where('type','preferences')
        ->map(function($d) {
            return [
                'name'=>$d->name,
                'value'=>$d->value
...
function getBoxaPrice($value) {
    $boxa_value = Session::get('BOXA_PRICE');
    $boxa_pricing = $value / $boxa_value;
    return number_format( $boxa_pricing + ($boxa_pricing * (env('BOXA_CHARGES') / 100)), 2);
}
function getBoxaPriceWihtoutFormat($value) {
    $boxa_value = Session::get('BOXA_PRICE');
    $boxa_pricing = $value / $boxa_value;
    return $boxa_pricing + ($boxa_pricing * (env('BOXA_CHARGES') / 100));
}
function getBoxaPriceFormatted($value) {    
    return 'BOXA '. getBoxaPrice($value);
}
...
{
    $position = Settings::all()->where('type','preferences')->where('name','money_format')->first()->value;
...
{
    $symbol = Currency::all()->firstWhere('code', $code)->symbol;
    return $symbol;
}
...
{
    $code = Currency::all()->firstWhere('symbol', $symbol)->code;
    return $code;
}
...
{
    $default_currency = Currency::all()->firstWhere('default', 1)->code;
    $rate = Currency::all()->firstWhere('code',$code)->rate;


    $base_amount = $field / $rate;


    $session_rate = Currency::all()->firstWhere('code', (Session::get('currency')) ? Session::get('currency') : $default_currency)->rate;
...
function getInboxUnreadCount() {
    return DB::table(DB::raw("(SELECT booking_id from messages where receiver_id=".Auth::id()." and `read`=0 ORDER by id DESC) as msg"))
        ->groupBy('booking_id')
        ->get()->count();
}


function convert_currency($fromParam = '', $toParam = '', $priceParam = null) {

    $from = currentCurrency($fromParam);
    $to = currentCurrency($toParam);
    $price       = str_replace(']','',$priceParam);//For Php Version 7.2
    $base_amount = (float) $price / $from->rate;
    return round($base_amount * $to->rate, $to->rate > 1000 ? 0 : 2);
}

function defaultCurrency() {
    return Currency::all()->firstWhere('default', 1);
}

function currentCurrency($code = null) {
    if($code && $code <> '') {
        return Currency::all()->firstWhere('code', $code);
    } elseif(\Session::get('currency')) {
        return Currency::all()->firstWhere('code', \Session::get('currency'));
    }
    return Currency::all()->firstWhere('default', 1);
}
