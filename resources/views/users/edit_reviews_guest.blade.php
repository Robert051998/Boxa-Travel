                ...
                <div class="row m-0 mb-3">
					<div class="col-12 p-0">
						<div class="d-flex justify-content-between align-items-center">
							<h4 class="m-0">{{ trans('messages.reviews.write_review_for') }} {{ $result->properties->users->first_name }}</h4>
						</div>
					</div>
				</div>
						...
						<div class=" {{ $review_id?'':'display-off'}} review-div booking-details mr-5">
							...
							<form id="guest-review-form1" method="post" class="edit_review">
								...
								<div id="review-guest-1">
									...
									<div class="form-group">
										<label>{{ trans('messages.reviews.describe_your_exp') }} <span class="text-danger">*</span></label>
										<p>{{ trans('messages.reviews.describe_your_exp_guest_desc',['site_name'=>$site_name]) }}</p>
										<textarea rows="5" placeholder="{{ trans('messages.reviews.describe_your_exp_placeholder') }}" name="message" id="message" data-behavior="countable" cols="40" maxlength="500" class="form-control">{{ isset($result->review_details($review_id)->message) ? $result->review_details($review_id)->message : '' }}</textarea>
										<span>{{ trans('messages.reviews.500_words_left') }}</span>
										<span class="error-msg d-none text-danger" id='error-comments'>{{ trans('messages.reviews.this_field_is_required') }}</span>
									</div>

									<div class="form-group">
										<label>{{ trans('messages.reviews.private_host_feedback') }}</label>
										<p>{{ trans('messages.reviews.private_host_feedback_desc',['site_name'=>$site_name]) }}</p>
										<div class="row-space-2">
											<label>{{ trans('messages.reviews.what_did_you_love_about_staying') }}</label>
											<textarea rows="5" name="secret_feedback" id="secret_feedback" class="form-control">{{ isset($result->review_details($review_id)->secret_feedback) ? $result->review_details($review_id)->secret_feedback : '' }}</textarea>
										</div>
										<div class="mt-4">
											<label>{{ trans('messages.reviews.how_host_improve') }}</label>
											<textarea rows="5" name="improve_message" id="improve_message" class="form-control">{{ isset($result->review_details($review_id)->improve_message) ? $result->review_details($review_id)->improve_message : '' }}</textarea>
										</div>
									</div>

									<div class="form-group">
										<label>{{ trans('messages.reviews.overall_exp') }}<span class="text-danger">*</span></label>
									...
									<div class="form-group">
										<button class="btn btn-outline-danger mt-3" type="submit" id="btn_next">
							...
							<form id="guest-review-form2" method="post" class="edit_review">
								...
								<div id="review-guest-2" class="display-off">
									<div class="form-group">
										<label>{{ trans('messages.reviews.accuracy') }}</label>
										<p>{!! trans('messages.reviews.accuracy_desc1') !!}</p>
									...
									<div class="form-group">
										<label>{{ trans('messages.reviews.cleanliness') }}</label>
										<p>{{ trans('messages.reviews.cleanliness_desc1') }}</p>
									...
									<div class="form-group">
										<label>{{ trans('messages.reviews.arrival') }}</label>
										<p>{{ trans('messages.reviews.arrival_desc1') }}</p>
									...
									<div class="form-group">
										<label>{{ trans('messages.reviews.amenities') }}</label>
										<p>{{ trans('messages.reviews.amenities_desc1') }}</p>
									...
									<div class="form-group">
										<label>{{ trans('messages.reviews.communication') }}</label>
										<p>{{ trans('messages.reviews.communication_desc1') }}</p>
									...
									<div class="form-group">
										<label>{{ trans('messages.reviews.location') }}</label>
										<p>{{ trans('messages.reviews.location_desc1') }}</p>
									...
									<div class="form-group">
										<label>{{ trans('messages.reviews.value') }}</label>
										<p>{{ trans('messages.reviews.value_desc1') }}</p>
									...
									<div class="form-group">
										<button class="btn btn-outline-danger mt-3" id="btn_submit" type="submit">
							...
							<a href="{{ url('/') }}/properties/{{ $result->property_id }}" alt="{{ $result->properties->name }}" class="media-cover">
								<div class="user-details">
									...
									<div class="user-name mt-2">
										...
										{{ trans('messages.reviews.hosted_by') }} <br />
										{{ $result->dates }}
									</div>
								</div>
							</a> 
							<a href="{{ url('/') }}/users/show/{{ $result->host_id }}" class="media-cover" target="_blank">{{ $result->properties->users->full_name }}</a>
							<div class="user-name mt-2">
								{{ $result->dates }}
							</div>
						</div>
