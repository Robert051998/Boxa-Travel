						...
						<div class="inner-main-panel">
							<form id="lis_pricing" method="post" action="{{url('listing/'.$result->id.'/'.$step)}}" accept-charset='UTF-8'>
								...
								<div class="inner-inner-form-row row">
									<div class="col-12">
										<h3 class="mt-5 mb-5">{{trans('messages.listing_price.base_price')}}</h3>
									</div>
									...
									<div class="col-lg-6">
										...
										<div class="form-group w-100">
											<div class="input-group-prepend ">
												<span class="input-group-text">{!! $result->property_price->currency->org_symbol !!}</span>
												<input type="text" id="price-night" value="{{ ($result->property_price->original_price == 0) ? '' : $result->property_price->original_price }}" name="price"  class="money-input w-100" >
											</div>
									...
									<div class="col-md-12">
										@if($result->property_price->weekly_discount == 0 && $result->property_price->monthly_discount == 0)
											<p id="js-set-long-term-prices" class="text-muted set-long-term-prices fs-6">
											{{trans('messages.listing_price.access_offer')}}  
											{{trans('messages.listing_price.week_month')}}
											{{trans('messages.listing_price.price')}}.
											</p>
										@endif
									</div>
								</div>
								<div class="form-row mt-4 border rounded pb-4 m-0  {{ ($result->property_price->weekly_discount == 0 && $result->property_price->monthly_discount == 0)? 'display-off':''}}" id="long-term-div">
									...
									<div class="col-md-12 pl-5 pr-5">
										...
										<div class="input-addon">
											<input type="text" data-suggested="" id="price-week" class="text-16 form-control" value="{{ $result->property_price->weekly_discount }}" name="weekly_discount" data-saving="long_price">
									...
									<div class="col-md-12 mt-3 pl-5 pr-5">
										...
										<div class="input-addon">
											<input type="text" data-suggested="₹16905" id="price-month" class="form-control text-16 mt-2" value="{{ $result->property_price->monthly_discount }}" name="monthly_discount" data-saving="long_price">
								...
								<div class="mt-4 border rounded pb-4 m-0">
									...
									<div id="additional-guests" class="{{($result->property_price->original_guest_fee == 0)?'display-off':''}}">
										<div class="col-md-12 pl-3 pr-3 pl-sm-5 pr-sm-5 mt-3">
											...
											<div class="input-group mt-3">
												<select id="price-select-guests_included" name="guest_after" data-saving="additional-saving" class="text-16 form-control">
													@for($i=1;$i<=16;$i++)
										...
										<div class="col-md-12 pl-3 pr-3 pl-sm-5 pr-sm-5 mt-3">
											<div class="input-group">
												<div class="input-group mb-3">
													<div class="input-group-prepend">
														<span class="input-group-text text-16">{!! $result->property_price->currency->org_symbol !!}</span>
														<input type="text" data-extras="true" value="{{ $result->property_price->original_weekend_price }}" id="price-weekend" name="weekend_price" class="text-16 money-input" data-saving="additional-saving">
													</div>
												</div>
											</div>
										</div>
									...
									<div class="col-md-12 pl-3 pr-3 pl-sm-5 pr-sm-5 mt-4">
										<label for="listing_cleaning_fee_native_checkbox" class="label-large label-inline">
											<input type="checkbox" class="pricing_checkbox" data-rel="pet_price" {{($result->property_price->pet_price == 0)?'':'checked="checked"'}}>
											Additional Pet Pricing
										</label>
									</div>

									<div id="pet_price" class="{{($result->property_price->pet_price == 0)?'display-off':''}}">
										<div class="col-md-12 pl-3 pr-3 pl-sm-5 pr-sm-5 mt-3">
											<div class="input-group">
												<div class="input-group mb-3">
													<div class="input-group-prepend">
														<span class="input-group-text text-16">{!! $result->property_price->currency->org_symbol !!}</span>
														<input type="text" data-extras="true" value="{{ $result->property_price->pet_price }}" id="price-weekend" name="pet_price" class="text-16 money-input" data-saving="additional-saving">
													</div>
												</div>
											</div>
										</div>
								...
								<div class="row justify-content-between mt-4 mb-5">
									<div class="mt-4">
										<a  data-prevent-default="" href="{{ url('listing/'.$result->id.'/photos') }}" class="btn btn-outline-danger secondary-text-color-hover font-weight-700 pl-5 pr-5 pt-3 pb-3 pl-5 pr-5">
											{{trans('messages.listing_description.back')}}
										</a>
									</div>

									<div class="mt-4">
										<button type="submit" class="btn vbtn-outline-success font-weight-700 pl-5 pr-5 pt-3 pb-3 pl-5 pr-5" id="btn_next"> <i class="spinner fa fa-spinner fa-spin d-none" ></i> <span id="btn_next-text">{{trans('messages.listing_basic.next')}}</span>
										</button>
									</div>
								</div>
							</form>
