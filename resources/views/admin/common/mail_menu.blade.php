<div class="box">
	<div class="box-body no-padding d-block">
		<ul class="nav nav-pills nav-stacked">
			...
			<!-- @if(Helpers::has_permission(Auth::guard('admin')->user()->id, 'manage_email_template'))
				<li class="admin-none" {{ isset($list_menu) &&  $list_menu == 'menu-8' ? 'class=active ' : ''}}><a href="{{ URL::to("admin/email-template/8")}}">Payout Sent</a></li>
			@endif -->
			...
			@endif -->
			@if(Helpers::has_permission(Auth::guard('admin')->user()->id, 'manage_email_template'))
				<li {{ isset($list_menu) &&  $list_menu == 'menu-11' ? 'class=active' : ''}}><a href="{{ URL::to("admin/email-template/11")}}">Booking Confirmation</a></li>
			@endif
		</ul>
	</div>
</div>